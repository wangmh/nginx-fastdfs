...
static char *ngx_http_fastdfs_set(ngx_conf_t *cf, ngx_command_t *cmd,
		void *conf);
...
static ngx_int_t ngx_http_fastdfs_proxy_process_status_line(
...
static char *ngx_http_fastdfs_merge_loc_conf(ngx_conf_t *cf, void *parent,
		void *child);
...
static ngx_str_t ngx_http_proxy_hide_headers[] = { ngx_string("Date"),
		ngx_string("Server"), ngx_string("X-Pad"),
		ngx_string("X-Accel-Expires"), ngx_string("X-Accel-Redirect"),
				ngx_string("X-Accel-Limit-Rate"),
				ngx_string("X-Accel-Buffering"), ngx_string("X-Accel-Charset"),
				ngx_null_string };

/* Commands */
static ngx_command_t ngx_http_fastdfs_commands[] = { {
		ngx_string("ngx_fastdfs_module"), NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS,
		ngx_http_fastdfs_set, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },

ngx_null_command };

static ngx_http_module_t ngx_http_fastdfs_module_ctx = { NULL, /* preconfiguration */
...
/* hook */
ngx_module_t ngx_http_fastdfs_module = { NGX_MODULE_V1,
...
static int fdfs_send_file(void *arg, const char *filename,
	...
	if (ngx_open_cached_file(ccf->open_file_cache, &ngx_filename, &of,
			r->pool) != NGX_OK) {
		switch (of.err) {
		case 0:
			return NGX_HTTP_INTERNAL_SERVER_ERROR;
		...
		if (rc != NGX_HTTP_NOT_FOUND || ccf->log_not_found) {
			ngx_log_error(level, r->connection->log, of.err, "%s \"%s\" failed",
					of.failed, filename);
		}
...
#define FDFS_REDIRECT_PARAM  "redirect=1"
	...
	for (i = 0; /* void */; i++) {
		...
		len += header[i].key.len + 2 + header[i].value.len + sizeof(CRLF) - 1;
	}
...
static void ngx_http_fastdfs_proxy_finalize_request(ngx_http_request_t *r,
		ngx_int_t rc) {
	ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
...
static int ngx_http_fastdfs_proxy_handler(void *arg, const char *dest_ip_addr) {
	...
	u->resolved->port = (in_port_t) ntohs(
			((struct sockaddr_in *) r->connection->local_sockaddr)->sin_port);
